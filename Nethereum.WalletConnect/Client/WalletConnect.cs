...
using Nethereum.WalletConnect.Events;
using Nethereum.WalletConnect.Events.Request;
...
namespace Nethereum.WalletConnect.Client
{
    public class WalletConnect : IDisposable
    {
        ...
        private async Task<WCSessionData> CreateSession()
        {
            ...
            TaskCompletionSource<WCSessionData> eventCompleted = new TaskCompletionSource<WCSessionData>(TaskCreationOptions.None);

            //Listen for the _handshakeId response
            //The response will be of type WCSessionRequestResponse
            Events.ListenFor<WCSessionRequestResponse>(this._handshakeId.ToString(), HandleSessionResponse);
        ...
        public WalletConnectClient CreateProvider()
        {
            if (!Connected)
            {
            ...
            return new WalletConnectClient(this);
        }
    }
}